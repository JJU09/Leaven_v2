...
import { DeductionOverride, PayrollRecord } from "@/features/payroll/types";
...
export type AllowanceField = "overtime_pay" | "weekly_holiday_pay";
...
interface AllowanceEditorProps {
  ...
  onOverrideSubmit: (
    ...
    shouldRecalculateDeductions: boolean
  ) => void;
}
...
export function AllowanceEditor({
  ...
  const [editingField, setEditingField] = useState<AllowanceField | null>(null);
  const [editValue, setEditValue] = useState<string>("");
  const [reason, setReason] = useState<string>("");
  const [shouldRecalculate, setShouldRecalculate] = useState<boolean>(true);

  const safeOverrides = record.overrides || [];
  ...
  const handleEditClick = (field: AllowanceField, value: number) => {
    if (isLocked) return;
  ...
  const handleSave = () => {
    ...
    onOverrideSubmit(
      {
        field: editingField as any, // DB에는 overtime_pay, weekly_holiday_pay로 텍스트 저장됨
        originalValue: record[editingField],
        overriddenValue,
        reason,
      },
      shouldRecalculate
  ...
  const fields: { key: AllowanceField; label: string }[] = [
    { key: "overtime_pay", label: "연장근무수당" },
    ...
    <div className="space-y-3 w-full">
      <div className="grid grid-cols-2 gap-3 w-full">
        {fields.map(({ key, label }) => {
          ...
          // 편집 중일 때 원래 값 찾기 (override가 있으면 originalValue, 없으면 현재 value)
          const overrideRecord = safeOverrides.find(o => o.field === key);
          const original = overrideRecord ? overrideRecord.originalValue : value;

          return (
            <div key={key} className="col-span-2 sm:col-span-1 flex flex-col rounded-lg border border-slate-200 p-4 bg-white h-full">
                ...
                <div className="space-y-3 mt-3 bg-slate-50/50 p-3 rounded-md border border-slate-100">
                  ...
                  <div className="flex items-start space-x-2 pt-1 pb-1">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => setShouldRecalculate(checked as boolean)}
                      className="mt-0.5"
                    />
                    <label
                      htmlFor="recalculate"
                      className="text-xs font-medium leading-relaxed peer-disabled:cursor-not-allowed peer-disabled:opacity-70 cursor-pointer text-slate-700"
                    >
                      공제액 자동 재계산
                    </label>
                  </div>

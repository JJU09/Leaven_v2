...
export function usePayroll(storeId: string | undefined, year: number, month: number) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // 3. 권한이 있는 경우에만 급여 데이터 자동 갱신 및 생성 로직 실행
      if (hasManageSalaryPerm) {
        // 3-1. 해당 기간의 모든 직원 스케줄 데이터 조회 (기존 데이터 업데이트 + 신규 데이터 생성용)
        ...
        // 3-2. 기존 미확정(draft) 상태인 급여 레코드들의 근무시간 재계산 및 DB 업데이트
        if (data) {
